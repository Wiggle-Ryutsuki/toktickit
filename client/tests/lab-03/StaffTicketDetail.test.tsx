...
const mockStaffAssignees = [
  { id: 7, displayName: "Somchai Staff", email: "staff.somchai@kmutt.ac.th", role: "IT_STAFF" as const },
  { id: 8, displayName: "Malee Jaidee", email: "staff.malee@kmutt.ac.th", role: "IT_STAFF" as const },
  { id: 9, displayName: "Admin TokTickIT", email: "admin.toktickit@kmutt.ac.th", role: "ADMINISTRATOR" as const },
];

const mockTicketData: api.TicketDetailFullDto = {
  id: 101,
...
describe("IT Staff Ticket Detail Component (UI-06, UI-07)", () => {
  ...
  function renderStaffDetail(ticketId = 101) {
    return render(
      <AuthContext.Provider value={mockContext}>
        <RequesterProvider>
          <StaffTicketDetail ticketId={ticketId} onBack={mockOnBack} />
        </RequesterProvider>
  ...
  describe("UI-06: IT Staff Ticket Detail Operational Controls", () => {
    ...
    it("UI-06.2: Renders operational controls: Owner dropdown, IT Priority dropdown, and Status transitions", async () => {
      ...
      const statusSelect = screen.getByTestId("status-transition-select") as HTMLSelectElement;
    ...
    it("UI-06.4: Enforces resolution summary when transitioning to RESOLVED or CLOSED", async () => {
      ...
      expect(await screen.findByTestId("resolution-summary-error")).toHaveTextContent(
        /Resolution summary is required and must be between 10 and 2000 characters/i
      );
    ...
    it("UI-06.5: Optimistic concurrency conflict displays error banner and reloads latest ticket state", async () => {
      ...
      const conflictError = new Error("Ticket has been modified by another user.") as any;
  ...
  describe("UI-07: Comments & Notes Visual Distinction & Segregation", () => {
    ...
    it("UI-07.3: Internal notes thread renders in amber warning style, displays confidential warning banner, and posts note", async () => {
      ...
      const newNoteData = {
        ...
        ticketId: 101,
        authorId: mockStaffUser.id,
        commentType: "INTERNAL_NOTE" as const,
