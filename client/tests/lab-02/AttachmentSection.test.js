...
describe("UI-08: Attachment Section Component", () => {
    ...
    it("UI-08.1: renders active attachments with filename, formatted size, and action buttons", () => {
        render(_jsx(RequesterProvider, { children: _jsx(AttachmentSection, { ticketId: 101, attachments: mockAttachments, onAttachmentChanged: vi.fn() }) }));
    ...
    it("UI-08.4: submitting valid removal reason calls DELETE API and closes modal", async () => {
        ...
        render(_jsx(RequesterProvider, { children: _jsx(AttachmentSection, { ticketId: 101, attachments: mockAttachments, onAttachmentChanged: onAttachmentChangedMock }) }));
        const removeBtn = screen.getByRole("button", { name: /Remove/i });
        fireEvent.click(removeBtn);
        ...
        fireEvent.click(confirmBtn);
        await waitFor(() => {
            expect(globalThis.fetch).toHaveBeenCalledWith(expect.stringContaining("/api/tickets/101/attachments/1"), expect.objectContaining({ method: "DELETE" }));
    ...
    it("UI-08.5: displays active attachment count and hides upload form when limit of 5 is reached", () => {
        ...
        render(_jsx(RequesterProvider, { children: _jsx(AttachmentSection, { ticketId: 101, attachments: fiveActiveAttachments, onAttachmentChanged: vi.fn() }) }));
    ...
    it("UI-08.6: uploading a valid file sends multipart request and invokes onAttachmentChanged", async () => {
        ...
        await waitFor(() => {
            expect(globalThis.fetch).toHaveBeenCalledWith(expect.stringContaining("/api/tickets/101/attachments"), expect.objectContaining({ method: "POST" }));
            expect(onAttachmentChangedMock).toHaveBeenCalled();
        });
    });
    ...
    it("UI-08.8: renders empty attachment state when ticket has 0 attachments", () => {
        render(_jsx(RequesterProvider, { children: _jsx(AttachmentSection, { ticketId: 101, attachments: [], onAttachmentChanged: vi.fn() }) }));
