...
describe("Create Ticket Screen (UI-02, UI-03, UI-04, UI-09)", () => {
  beforeEach(() => {
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (url) => {
      ...
      if (urlStr.includes("/api/requesters")) {
        return { ok: true, json: async () => [mockActiveRequester] } as Response;
      }
  ...
  it("UI-03: validates required fields on client submission without calling API", async () => {
    ...
    const ticketPostCalls = fetchSpy.mock.calls.filter(([url]) =>
  ...
  it("UI-04: shows busy spinner during submission and displays success banner with Ticket Number", async () => {
    vi.spyOn(globalThis, "fetch").mockImplementation(async (url, init) => {
      const urlStr = String(url);
      if (urlStr.includes("/api/categories")) return { ok: true, json: async () => mockCategories } as Response;
      if (urlStr.includes("/api/related-systems")) return { ok: true, json: async () => mockSystems } as Response;
